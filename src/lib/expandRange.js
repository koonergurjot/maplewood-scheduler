 ...
 */
export function expandRangeToVacancies(range, awardAsBlock = true) {
    ...
    if (bundleId)
        console.debug("[bundle] created", bundleId, { days: days.length });
    return days.map((d) => ({
        id: crypto.randomUUID(),
        ...(singleAward
            ? { bundleId, bundleMode: "one-person" }
            : {}),
        reason: range.reason,
        ...
        shiftDate: d,
        shiftStart: range.perDayTimes?.[d]?.start ?? range.shiftStart ?? "06:30",
