...
export function ensureBundleId(v) {
    if (!v.bundleId)
        v.bundleId = crypto.randomUUID();
    return v.bundleId;
}
...
// the entire block.
export function bundleContiguousVacanciesByRef(vacs) {
    const byRef = new Map();
    for (const v of vacs) {
        if (!v.vacancyRef)
            continue;
