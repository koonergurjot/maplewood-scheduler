...
export function BidsPage({ bids, archivedBids, setBids, vacancies, vacations, employees, employeesById, }) {
    ...
    const filteredVacancyOptions = useMemo(() => {
        return openVacancyOptions.filter((opt) => {
            if (!matchText(vacancyFilter, opt.label))
                return false;
            if (onlyEligible && selectedEmployee) {
                const vac = vacancies.find((v) => v.id === opt.id);
                if (vac && !vacancyEligible(vac, selectedEmployee))
                    return false;
            }
            return true;
        });
    ...
    return (_jsxs("div", { className: "grid", children: [_jsxs("div", { className: "card", children: [_jsx("div", { className: "card-h", children: "Add Bid" }), _jsx("div", { className: "card-c", children: _jsxs("div", { className: "row cols2", children: [_jsxs("div", { style: { gridColumn: "1 / -1" }, children: [_jsx("label", { children: "Employee" }), _jsx(EmployeeCombo, { employees: employees, onSelect: (id) => {
                                                ...
                                                setNewBid((b) => {
                                                    ...
                                                    if (e) {
                                                        ...
                                                        if (bad.length) {
                                                            window.appShowAlert?.("Warning: employee may be ineligible for some selected vacancies");
                                                        }
                                                    }
                                                    return updated;
                                                });
                                            } }, bidFormKey), _jsx("div", { className: "subtitle", style: { marginTop: 4 }, children: "Select an employee to highlight matching vacancies below." })] }), _jsxs("div", { style: { gridColumn: "1 / -1" }, children: [_jsx("label", { children: "Vacancies" }), _jsx("div", { className: "subtitle", style: { marginBottom: 4 }, children: "Eligible vacancies are highlighted; use the checkbox to filter." }), _jsx("input", { type: "text", placeholder: "Filter vacancies\u2026", value: vacancyFilter, onChange: (e) => setVacancyFilter(e.target.value) }), _jsxs("div", { style: {
                                                display: "flex",
                                                gap: 8,
                                                alignItems: "center",
                                                margin: "4px 0",
                                                flexWrap: "wrap",
                                            }, children: [_jsxs("span", { children: [newBid.selectedVacancyIds.length, " selected"] }), _jsxs("label", { children: [_jsx("input", { type: "checkbox", checked: onlyEligible, onChange: (e) => setOnlyEligible(e.target.checked) }), "Eligible only"] }), _jsx("button", { className: "btn btn-sm", onClick: () => setNewBid((b) => ({
                                                        ...b,
                                                ...
                                                padding: 4,
                                                borderRadius: 4,
                                            }, children: filteredVacancyOptions.length ? (filteredVacancyOptions.map((opt) => {
                                                const vac = vacancies.find((x) => x.id === opt.id);
                                                const eligible = vac ? vacancyEligible(vac, selectedEmployee) : true;
                                                return (_jsxs("label", { style: { display: "block", opacity: eligible ? 1 : 0.5 }, children: [_jsx("input", { type: "checkbox", checked: newBid.selectedVacancyIds.includes(opt.id), onChange: (e) => setNewBid((b) => ({
                                                                ...b,
