...
class SchedulerStateInsertConflictError extends Error {
  constructor(userId, facilityId) {
    super("scheduler-state-insert-conflict");
    this.name = "SchedulerStateInsertConflictError";
    this.userId = userId;
...
export async function upsertSchedulerState(userId, facilityId, expectedVersion, state) {
  ...
  try {
    return await withTransaction(async (client) => {
      ...
      if (existing.rowCount === 0) {
        ...
        try {
          ...
          if (error?.code === "23505") {
            throw new SchedulerStateInsertConflictError(userId, facilityId);
          }
          throw error;
        }
      }
    ...
    if (error instanceof SchedulerStateInsertConflictError) {
      const pool = getDbPool();
      const latest = await pool.query(
        `SELECT version, updated_at
         FROM scheduler_states
         WHERE user_id = $1 AND facility_id = $2`,
        [error.userId, error.facilityId],
      );
